...
if(rx_flag) {
    for(uint32_t i = 0; i < rx_index; i++) {
        buffer[i] = rx_buffer[i];
    }
    buffer[rx_index] = 0;  // Null terminate the string
...
// Check for invalid characters
		else if (buffer[0] != 'Q' || buffer[0] != 'q' || buffer[0] != 'H' || buffer[0] != 'h' || buffer[0] != 'S' || buffer[0] != 's' || buffer[0] != '1' ||buffer[0] != '0' && strlen((char*)buffer) == 1)
		{
			message("\r\n\Invalid Entry!\r\n");
			message("> ");
		}
